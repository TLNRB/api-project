...
// Settings
const TOTAL_DURATION_MINUTES = 120; // How long we keep the server running
const MINUTES_DELTA = 5; // How often we ping the server
const cronPattern = "*/" + MINUTES_DELTA + " * * * *"; // Docs here: https://crontab.guru/#*/5_*_*_*_*
const URL = "https://api-project-4z46.onrender.com/api/";

let counter = 0;
let task: ScheduledTask;

// Small helper function to ping the server
function pingServer() {
   https.get(URL, () => {
...
export async function startCron(req: Request, res: Response) {
   try {
      ...
      res.status(200).send("Cron job started from active route call!");
   }
   catch (err) {
